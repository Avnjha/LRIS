...
class _AddLostItemScreenState extends State<AddLostItemScreen> {
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      if (_lostDate == null) {
        Helpers.showToast('Please select lost date', isError: true);
        return;
      }
      ...
      final success = await itemProvider.createLostItem(
      ...
      if (success) {
        Helpers.showToast('Lost item reported successfully!');
        Navigator.pop(context);
      } else {
        Helpers.showToast(
          itemProvider.error ?? 'Failed to report lost item',
          isError: true,
        );
      }
    }
                ...
                child: Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      Text(
                        _lostDate != null
                            ? Helpers.formatDate(_lostDate!)
                            : 'Select Date',
                        style: TextStyle(
                          fontSize: 16,
                          color: _lostDate != null ? Colors.black : Colors.grey,
                        ),
                      ),
  ...
  void _showHelpDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Tips for Reporting Lost Items'),
        content: SingleChildScrollView(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            mainAxisSize: MainAxisSize.min,
            children: const [
              Text('• Be specific about the location'),
              Text('• Provide detailed description'),
              Text('• Include brand and color if known'),
              Text('• Add clear photos if available'),
